...
impl NativeGraph {
    ...
    fn add_node(&mut self, id: String, kind: String, name: String) {
        self.inner.add_node(Node { id, kind, name, properties: serde_json::json!({}) });
    }
    ...
    fn add_edge(&mut self, source: String, kind: String, target: String) -> PyResult<()> {
        self.inner
            .add_edge(Edge { source, kind, target, properties: serde_json::json!({}) })
            .map_err(pyo3::exceptions::PyValueError::new_err)
    }
    ...
    fn add_edge_json(&mut self, source: String, kind: String, target: String, properties_json: String) -> PyResult<()> {
        let properties = serde_json::from_str(&properties_json)
            .map_err(pyo3::exceptions::PyValueError::new_err)?;
        self.inner
            .add_edge(Edge { source, kind, target, properties })
            .map_err(pyo3::exceptions::PyValueError::new_err)
    }
    ...
    fn policy_findings_json(&self, max_depth: usize) -> PyResult<String> {
        serde_json::to_string(&self.inner.policy_findings(max_depth)).map_err(pyo3::exceptions::PyValueError::new_err)
    }

    fn attack_paths_json(&self, max_depth: usize) -> PyResult<String> {
        serde_json::to_string(&self.inner.attack_paths(max_depth)).map_err(pyo3::exceptions::PyValueError::new_err)
    }

    fn blast_radius_json(&self, max_depth: usize) -> PyResult<String> {
        serde_json::to_string(&self.inner.blast_radius(max_depth)).map_err(pyo3::exceptions::PyValueError::new_err)
    }

    fn export_json(&self) -> PyResult<String> {
        self.inner.export_json().map_err(pyo3::exceptions::PyValueError::new_err)
    }
