...
impl NativeGraph {
    ...
    fn parse_authorization_json(&self, provider: String, payload: String) -> PyResult<String> { let model = match provider.as_str() { "aws-iam" => agentbom_engine::parse_aws_iam(&payload), "gcp-iam" => agentbom_engine::parse_gcp_iam(&payload), "azure-rbac" => agentbom_engine::parse_azure_rbac(&payload), "kubernetes-rbac" => agentbom_engine::parse_kubernetes_rbac(&payload), "oauth" | "oauth-scopes" => agentbom_engine::parse_oauth_scopes(&payload), "mcp" | "mcp-auth" => agentbom_engine::parse_mcp_auth(&payload), _ => return Err(pyo3::exceptions::PyValueError::new_err(format!("unsupported authorization provider: {provider}"))), }.map_err(pyo3::exceptions::PyValueError::new_err)?; serde_json::to_string(&model).map_err(pyo3::exceptions::PyValueError::new_err) }
    fn monitor_runtime_json(&self, declared_json: String, events_json: String) -> PyResult<String> { let declared: Vec<String> = serde_json::from_str(&declared_json).map_err(pyo3::exceptions::PyValueError::new_err)?; let events: Vec<RuntimeEvent> = serde_json::from_str(&events_json).map_err(pyo3::exceptions::PyValueError::new_err)?; let monitor = agentbom_engine::RuntimeMonitor::new(declared); let findings = events.into_iter().filter_map(|event| monitor.observe(event)).collect::<Vec<_>>(); serde_json::to_string(&findings).map_err(pyo3::exceptions::PyValueError::new_err) }
    ...
    fn delegation_findings_json(&self, principal: String, max_hops: usize) -> PyResult<String> { serde_json::to_string(&self.inner.delegation_findings(&principal, max_hops)).map_err(pyo3::exceptions::PyValueError::new_err) }
